...
with tab1:
    ...
    with st.expander("1️⃣ Earthquakes per Year"):
        ...
        st.dataframe(res)
        st.plotly_chart(px.bar(res, x='year', y='count'), use_container_width=True, key="q1")

    with st.expander("2️⃣ Earthquakes per Month"):
        res = df.groupby('month').size().reset_index(name='count')
        st.dataframe(res)

    with st.expander("3️⃣ Day-wise Distribution"):
        st.dataframe(df.groupby('day').size().reset_index(name='count'))

    with st.expander("4️⃣ Total Tsunami Events"):
        st.success(int(df['tsunami'].sum()))

    with st.expander("5️⃣ Average Magnitude"):
        st.success(round(df['mag'].mean(), 2))

# ==================================================
# TAB 2: MAGNITUDE & DEPTH (6–12)
# ==================================================
with tab2:
    ...
    with st.expander("9️⃣ Average Magnitude by Type"):
        st.dataframe(df.groupby('magType')['mag'].mean().round(2).reset_index())

    with st.expander("🔟 Average Depth by Type"):
        st.dataframe(df.groupby('magType')['depth_km'].mean().round(2).reset_index())
    ...
    with st.expander("1️⃣2️⃣ Shallow vs Deep Count"):
        st.write({
            "Shallow": int((df['depth_km']<70).sum()),
            "Deep": int((df['depth_km']>300).sum())
        })

# ==================================================
# TAB 3: TIME ANALYSIS (13–18)
# ==================================================
with tab3:
    with st.expander("1️⃣3️⃣ Earthquakes per Hour"):
        res = df.groupby('hour').size().reset_index(name='count')
        st.dataframe(res)
        st.plotly_chart(px.bar(res, x='hour', y='count'), use_container_width=True, key="q13")

    with st.expander("1️⃣4️⃣ Month with Highest Earthquakes"):
        st.success(int(df.groupby('month').size().idxmax()))

    with st.expander("1️⃣5️⃣ Year with Highest Earthquakes"):
        st.success(int(df.groupby('year').size().idxmax()))

    with st.expander("1️⃣6️⃣ Weekend vs Weekday"):
...
with tab4:
    ...
    with st.expander("2️⃣0️⃣ Tsunamis per Year"):
        st.dataframe(df[df['tsunami']==1].groupby('year').size().reset_index(name='count'))

    with st.expander("2️⃣1️⃣ Alert Distribution"):
        st.dataframe(df.groupby('alert').size().reset_index(name='count'))

    with st.expander("2️⃣2️⃣ Avg Magnitude by Alert"):
        st.dataframe(df.groupby('alert')['mag'].mean().round(2).reset_index())

    with st.expander("2️⃣3️⃣ High-Magnitude Tsunami (>7)"):
        st.dataframe(df[(df['tsunami']==1)&(df['mag']>7)])

    with st.expander("2️⃣4️⃣ % Events with Alerts"):
        st.success(round((df[df['alert']!='none'].shape[0]/df.shape[0])*100,2))

# ==================================================
# TAB 5: QUALITY & RISK (25–30)
# ==================================================
with tab5:
...
# ================= FOOTER =================
st.success("✅ Application loaded successfully — no MySQL, no cloud errors")